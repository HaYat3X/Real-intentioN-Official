...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $argument = $arr_prm_inst->staff_update_post_prm($userId, $type, $company, $format, $overview, $field, $time, $attachment, $post_id);

    // SQL発行
    $sql = 'UPDATE `staff_information_table` SET `staff_id`=?, `type`=?, `company`=?, `format`=?, `overview`=?, `field`=?, `time`=?, `attachment`=? WHERE post_id = ?';
                    ...
                    <div class="mx-auto col-lg-8 pt-2 pb-5">
                        <?php if (count($err_array) > 0) : ?>
                            <?php foreach ($err_array as $err_msg) : ?>
                                ...
                                <a class="login-btn btn" href="./post_form.php">戻る</a>
